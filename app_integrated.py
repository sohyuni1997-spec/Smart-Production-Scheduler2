    ...
    def fetch_db_data(user_input):
        ...
        try:
            ...
            if target_month and product_key is None and "비교" not in user_input and "초과" not in user_input and "사례" not in user_input:
                res = supabase.table("monthly_production").select("월, 총_생산량, 버전").eq("월", target_month).eq("버전", target_version).execute()
                ...
                if res.data:
                    ...
                    return f"{target_month}월 {target_version} 데이터를 찾을 수 없습니다."
